...
class Simple3DCNNActivityModel(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        # r3d_18: input [B, C, T, H, W]
        model3d = r3d_18(weights=R3D_18_Weights.DEFAULT)
        # drop its fc
        self.backbone3d = nn.Sequential(*list(model3d.children())[:-1])  # [B,512,1,1,1]
        feat_dim = 512
    ...
    def forward(self, x):
        ...
        feats = self.backbone3d(x3d).flatten(start_dim=1)  # [B,512]
        return self.classifier(feats)
    
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
num_classes = len(label_set)
model = Simple3DCNNActivityModel(num_classes=num_classes).to(device)
...
os.makedirs("3dcnn_resnet_checkpoints", exist_ok=True)
...
for epoch in range(total_epochs):
    ...
    ckpt_path = f"./3dcnn_resnet_checkpoints/ckpt_epoch_{epoch+1:02d}.pth"
