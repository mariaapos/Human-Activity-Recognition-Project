...
class ResNetLSTMActivityModel(nn.Module):
    def __init__(
        self,
        num_classes: int,
        hidden_size: int = 256,
        ...
        feat_dim = 512  # ResNet-18’s output channels
        ...
        self.classifier = ClassifierHead(in_dim=lstm_out_dim,
                                         num_classes=num_classes)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
      
        B, T, C, H, W = x.shape
        # collapse batch+time for 2D CNN
        x = x.view(B * T, C, H, W)

        # spatial features
        feat_map = self.backbone(x)            # [B*T, 512, 1, 1]
        pooled   = self.pool(feat_map)         # [B*T, 512, 1, 1]
        flat     = pooled.view(B, T, -1)       # [B, T, 512]

        # temporal encoding
        lstm_out, _ = self.lstm(flat)          # [B, T, hidden*dirs]

        # take last time step
        video_feat = lstm_out[:, -1, :]        # [B, hidden*dirs]
...
model = ResNetLSTMActivityModel(num_classes=num_classes).to(device)
...
test_loader  = DataLoader(test_ds, batch_size=8, num_workers=1, pin_memory=True, collate_fn=pad_collate)
...
# Load checkpoint
checkpoint_path = "./resnet_lstm_checkpoints/ckpt_epoch_40.pth"
