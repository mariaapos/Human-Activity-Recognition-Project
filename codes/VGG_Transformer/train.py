...
class VGGTransformerActivityModel(nn.Module):
    def __init__(self, num_classes, nhead=8, num_layers=1):
        super().__init__()
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=feat_dim, nhead=nhead, batch_first=False
        )
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.classifier = ClassifierHead(in_dim=feat_dim, num_classes=num_classes)
    ...
    def forward(self, x):
        ...
        pooled = self.avgpool(feat_map)       # [B*T, 512, 1, 1]
        flat = pooled.flatten(start_dim=1)    # [B*T, 512]
        seq = flat.view(B, T, -1)            # [B, T, 512]
        # transformer expects [S, B, E]
        seq_t = seq.permute(1, 0, 2)         # [T, B, 512]
        trans_out = self.transformer(seq_t)  # [T, B, 512]
        video_feat = trans_out[-1, :, :]     # [B, 512]
        return self.classifier(video_feat)
    
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
num_classes = len(label_set)
model = VGGTransformerActivityModel(num_classes=num_classes).to(device)
...
train_loader = DataLoader(train_ds, batch_size=2, shuffle=True, num_workers=8, pin_memory=True, collate_fn=pad_collate)
val_loader   = DataLoader(val_ds, batch_size=8, num_workers=4, pin_memory=True, collate_fn=pad_collate)

history = {'train_loss':[], 'val_loss':[], 'train_top1':[], 'val_top1':[],
           'train_top5':[], 'val_top5':[]}

os.makedirs("vgg_transformer_checkpoints", exist_ok=True)
...
for epoch in range(total_epochs):
    ...
    ckpt_path = f"./vgg_transformer_checkpoints/ckpt_epoch_{epoch+1:02d}.pth"
